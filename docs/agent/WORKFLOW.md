...
1. Перед архитектурными ответами, анализом поведения, нетривиальными изменениями, отладкой и редактированием прочитай `docs/agent/README.md`.
2. Затем прочитай релевантные файлы из `docs/agent/`: `PROJECT_MAP.md`, `INFRASTRUCTURE.md`, `WORKFLOW.md`, `CHANGELOG.md`.
3. Проверь статус:
...
4. Перед изменением поведения ищи по проекту через `rg`:

```sh
rg -n "term-to-find" .
```

5. Определи, какой файл документации нужно обновить:
...
- Не коммить generated files: `.next/`, `.wxt/`, app/package `dist/`, `runtime/source/*`, `runtime/tmp/*`, `runtime/output/*`, `runtime/downloads/*`, `runtime/compressed/*`, `runtime/obsidian/*`, кроме `.gitkeep`.
